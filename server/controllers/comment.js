...
export const postComment = async (req, res) => {
  ...
  try {
    const updatedQuestion = await Questions.findByIdAndUpdate(_id, {
      $addToSet: { comment: [{ commentBody, userCommented,userId: userId }] },
    });
...
export const postAnswerComment = async (req, res) => {
  const { id: _id } = req.params;
  const { ansId, anscommentBody, ansuserCommented ,userId} = req.body;
  ...
  try {
    const updatedAnswercom = await Questions.findByIdAndUpdate(_id, {
      $addToSet: {
        anscomment: [{ ansId, anscommentBody, ansuserCommented, userId:userId }],
      },
    });
